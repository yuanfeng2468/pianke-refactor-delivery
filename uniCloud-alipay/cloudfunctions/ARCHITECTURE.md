...
部署前必须在 uniCloud 控制台确认：`userAdCallback` 已上传到与广告位绑定的服务空间；`reconcileRewardOrders` 已配置并启用定时触发器；`cleanupFeedSessions` 与 `cleanupRateLimit` 仅接受运行时 `triggerType=timer`；`PIANKE_RECONCILE_TIMER_ENABLED=true`、`PIANKE_AD_CALLBACK_SECRET` 和管理密钥已配置；同一 `trans_id` 重试返回成功且不重复写入奖励；真实日志能看到 `reward_orders.status=rewarded` 和对应账本记录。对账任务遇到缺少官方 `trans_id` 的订单时必须保持等待外部证据，不得伪造回调。
